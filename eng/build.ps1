...
function Print-Usage() {
    ...
    Write-Host "  -UseArcade                Use the targets provided by the Arcade package; otherwise use the default implementations."
    Write-Host ""
...
function Process-Arguments() {
  ...
  if($sln -eq '')
  {
    $sln= Get-ChildItem -path "$PSScriptRoot\.." -filter "*.sln"
    $script:solution= $sln
  }
  Write-Host "Solution: '$sln'" -ForegroundColor Yellow
  if(-not (Test-Path $sln))  {
    throw "Can't find a project solution file."   
  }
  # Workarround end
  ...
  # 2023-11-29, Arkarin0: Remove once binary log is enabled.
  if($binaryLog){
    Write-Host "Binary log is not supported yet."
  }
  ...
  function BuildSolution-Core()
  {
    ...
    # 23-11-29, Arkarin0: Remove once Versioning is implemented.
    $args += " /p:DeleteMeArtifactsNonShippingPackagesDir=`"$PackagesDir\PerBuildRelease\Release`""
    $args += " /p:DeleteMeArtifactsShippingPackagesDir=`"$PackagesDir\Release\Release`""
    ...
    Write-Host "Path        : '$($projects)'"
    Write-Host "Confguration: " $configuration
    Write-Host "args        : " $args

    try {
     Run-MSBuild "$projects" $args
    }
    finally {
...
function TestUsingRunTestsArcade() {
