...
function GetPublishData() {
  ...
  #Arkarin0 2021-09-20: original
  #Write-Host "Downloading $PublishDataUrl"
  #$content = (Invoke-WebRequest -Uri $PublishDataUrl -UseBasicParsing).Content
  $publishDataFile = Join-Path $EngRoot "config/PublishData.json"
  $content =  Get-Content -Raw -Path $publishDataFile
...
function GetBranchPublishData([string]$branchName) {
  ...
  # 2023-11-29, Arkarin0: workarround, for when no name is provided. Is mostly the case for local builds.
...
#pack a csproj into a nuget package
function Pack-Nuget([string]$packageDir, [string]$packageProject, [string]$destination ="local", [string]$version, [string]$configuration="Debug"){
  ...
  Write-Host $currLoc
   ...
   try {
        $args = "/t:pack /p:NoWarn=1591"
 
     if ($destination -ne "local") {
       $args += " /p:PackageOutputPath=`"$destination`""
     }else {
       $destination = GetNuGetPackageCachePath
      $args += " /p:PackageOutputPath=`"$destination`""
     }
     ...
     Write-Host "Project     : " $packageProject
     Write-Host "Destination : " $destination
     Write-Host "Version : " $version
     Run-MSBuild `"$packageProject`" -configuration $configuration -warnAsError:$false -treatWarningAsError:$false -buildArgs $args
   }
   finally {
     Pop-Location
   }
...
function Run-MSBuild([string]$projectFilePath, [string]$buildArgs = "", [string]$logFileName = "", [switch]$parallel = $true, [switch]$summary = $true, [switch]$warnAsError = $true, [string]$configuration = $script:configuration, [switch]$runAnalyzers = $false, [switch] $treatWarningAsError = $true) {
  # Because we override the C#/VB toolset to build against our LKG package, it is important
  # that we do not reuse MSBuild nodes from other jobs/builds on the machine. Otherwise,
  # we'll run into issues such as https://github.com/dotnet/roslyn/issues/6211.
  # MSBuildAdditionalCommandLineArgs=
  $args = "/p:TreatWarningsAsErrors=$treatWarningAsError /nologo /nodeReuse:false /p:Configuration=$configuration ";
